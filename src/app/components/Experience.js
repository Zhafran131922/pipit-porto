        ...
        <div className="relative">
          ...
          >
            {experiences.map((exp, index) => (
              ...
              >
                {/* Timeline dot */}
                <div className={`absolute left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-6 h-6 ${exp.color} rounded-full z-10 flex items-center justify-center text-white shadow-lg`}>
                  {exp.icon}
                </div>
                
                {/* Content card */}
                <div className={`w-full md:w-5/12 p-6 rounded-xl shadow-xl backdrop-blur-sm ${
                  index % 2 === 0 
                    ? "bg-gradient-to-r from-blue-50 to-white ml-0 md:mr-auto" 
                    : "bg-gradient-to-r from-white to-blue-50 mr-0 md:ml-auto"
                } border border-gray-200`}>
                  <div className="flex items-start">
                    <div className={`${exp.color} w-12 h-12 rounded-lg flex items-center justify-center text-white text-xl mr-4`}>
                      {exp.icon}
                    </div>
                    <div>
